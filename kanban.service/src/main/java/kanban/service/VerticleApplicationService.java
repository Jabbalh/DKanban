...
public class VerticleApplicationService extends AbstractVerticle {
	...
	/**
	 * Renvois la liste simplifié des applications
	 * @param message
	 */
	/*
	private void appList(Message<String> message){		
		Async.When(()-> mongoService.findAll(ApplicationParameter.class, kanbanQuery()))		
		.doThat(x -> {
			List<ParamTuple> result = x.stream().map(a -> new ParamTuple(a.getCode(), a.getLibelle())).collect(Collectors.toList());
	...
	*/
	 ...
	 */
	/*
	private void zoneList(Message<String> message){
		Async.When(()-> mongoService.findAll(ZoneParameter.class, kanbanQuery().put("sort", "order")))
		.doThat(r -> message.reply(Json.encode(r.stream().skip(1).map(a -> new ParamTuple(a.getCode(), a.getLibelle())).collect(Collectors.toList()))));
	}
	*/
	...
	private void stateList(Message<String> message){
		Async.When(()-> mongoService.findAll(StatutParameter.class, kanbanQuery()))	
		.doThat(x -> {
			List<ParamTuple> result = x.stream()
						.map(a -> ParamTuple.from(a))
						.sorted((c1,c2) -> c1.safeCompare(c2))
	...
	*/
	...
	// A supprimer après insertion dans GIT
	/*
	private <T> Then<List<T>> extractInternalList(Class<T> clazz,String field){
		return Async.When(() -> mongoService.findInternListFromObject
				(
						KanbanParameter.class,
						clazz,
						new JsonObject().put("_id",KanbanParameter.EXPOSED_ID), 
						field
				));
	}
	*/
	/*
	private <T> Then<T> extractInternalField(Class<T> clazz,String field){
		return Async.When(() -> mongoService.findFieldFromEntity
				(
						KanbanParameter.class,
						clazz,
						new JsonObject().put("_id",KanbanParameter.EXPOSED_ID), 
	...
	*/
	/*
	private void saveStatut(Message<JsonObject> message){
		StatutParameter statut = Json.decodeValue(message.body().encode(), StatutParameter.class);
		
		Async.When(()-> mongoService.findAll(Ticket.class, new JsonObject().put("statut.code",statut.getCode())) )
		.doThat(tickets -> {
			
			for (Ticket item : tickets){
				item.setStatut(ParamTuple.from(statut));
				Async.When(()-> mongoService.update(
						DbUtils.index(Ticket.class), 
						new JsonObject().put("_id",item.get_id()), 
						new JsonObject().put("$set", new JsonObject().put("statut", new JsonObject(Json.encode(ParamTuple.from(statut)))))))
				.doThat(b -> {					
					vertx.eventBus().publish(EventBusNames.UPDATE_CARD, Json.encodePrettily(item));
				});
			}						
		...
		try
		{
			StatutParameter a = ApplicationData.get().getStatut().stream().filter(x -> x.getCode().equals(statut.getCode())).findFirst().get();
			a.setCode(statut.getCode());
			a.setDescription(statut.getDescription());
			a.setLibelle(statut.getLibelle());
			a.setColor(statut.getColor());
		}
		catch(Exception ex){
			ApplicationData.get().getStatut().add(statut);
		}
		
		
		Async.When(()-> mongoService.update(statut))
			.doThat(r -> message.reply(Json.encode(r)));
		
		
	}
	*/
	/*
	private <T extends AbstractParameter> void save(Message<JsonObject> message, Class<T> clazz){
		T entity = Json.decodeValue(message.body().encode(), clazz);
		entity.setKanbanParameter(KanbanParameter.EXPOSED_ID);
		Async.When(() -> mongoService.update(entity))
		.doThat(c -> message.reply(Json.encode(c)));
	}
	*/
	
	/*
	private void saveApplication(Message<JsonObject> message){
		
		ApplicationParameter application = Json.decodeValue(message.body().encode(), ApplicationParameter.class);
		Async.When(()-> mongoService.findAll(Ticket.class, new JsonObject().put("application.code",application.getCode())) )
		.doThat(tickets -> {
			
			for (Ticket item : tickets){
				item.setApplication(new ParamTuple(application.getCode(), application.getLibelle()));
				Async.When(()-> mongoService.update(
						DbUtils.index(Ticket.class), 
						new JsonObject().put("_id",item.get_id()), 
						new JsonObject().put("$set", new JsonObject().put("application", new JsonObject(Json.encode(ParamTuple.from(application)))))))
				.doThat(b -> {					
					vertx.eventBus().publish(EventBusNames.UPDATE_CARD, Json.encodePrettily(item));
				});
			}						
		...
		ApplicationParameter a = ApplicationData.get().getApplications().stream().filter(x -> x.getCode().equals(application.getCode())).findFirst().get();
		a.setCode(application.getCode());
		a.setDescription(application.getDescription());
		a.setLibelle(application.getLibelle());
		
		Async.When(()-> mongoService.update(application))
			.doThat(r -> message.reply(Json.encode(r)));
		
	}
	*/
	/*
	private void saveZoneApp(Message<JsonObject> message){
		
		message.reply(Json.encode(false));
	}
	*/
	
	/**
	 * Renvois la liste des applications (objet complet)
	 * @param message
	
	private void appListFull(Message<String> message){
		Async.When(()-> mongoService.findAll(ApplicationParameter.class,kanbanQuery()))
		.doThat(x -> {			
